...
public class LinearSceneOcTree<T> : LinearSceneTree<T> where T : ISceneObject,ISOLinkedListNode
{
    ...
    private bool InsertToNode(T obj, int depth, float centerx, float centery, float centerz, float sizex, float sizey,
        float sizez)
    {
        if (depth == m_MaxDepth)
        {
    ...
    public override void Trigger(IDetector detector, TriggerHandle<T> handle)
    {
	    ...
	    {
		    //如果使用相机裁剪,则计算出八个角点的裁剪掩码,且子节点的裁剪检测可以服用部分父节点的角点
		    //翻译: 就是计算是否被裁了,当Detector发生位置改变时必然触发一次
		    ...
		    TriggerToNodeByCamera(detector, handle, 0, code, m_Bounds.center.x, m_Bounds.center.y, m_Bounds.center.z,
			    m_Bounds.size.x, m_Bounds.size.y,
			    m_Bounds.size.z);
	    }
	    else
	    {
    ...
    private void TriggerToNodeByCamera(IDetector detector, TriggerHandle<T> handle, int depth,
	    TreeCullingCode cullingCode, float centerx, float centery, float centerz, float sizex, float sizey, float sizez)
    {
