...
namespace RVT
{
    ...
    public struct FDrawPageParameter
    {
    ...
    public class FPageRenderer : IDisposable
    {
        private int m_Limit;
        ...
        public void DrawPageTable(ScriptableRenderContext renderContext, CommandBuffer cmdBuffer,
            FPageProducer pageProducer)
        {
            m_Property.Clear();
            m_DrawInfos.Clear();
            ...
            //构建一个PageTable
            NativeArray<FPageTableInfo> pageTableInfos =
                new NativeArray<FPageTableInfo>(m_DrawInfos.Length, Allocator.TempJob);
            
            //构建pageTable,确定每个page的代销
        ...
        private void RenderPage(CommandBuffer cmdBuffer, VirtualTextureAsset virtualTexture, in FRectInt pageRect,
            in FPageLoadInfo loadRequest, in FDrawPageParameter drawPageParameter)
        {
            int x = loadRequest.x;
            int y = loadRequest.y;
            ...
            var padding = (int)virtualTexture.tileBorder * perSize *
                (drawPageParameter.volumeRect.width / virtualTexture.pageSize) / virtualTexture.tileSize;
            var volumeRect = new Rect(
                drawPageParameter.volumeRect.xMin +
                (float)x / virtualTexture.pageSize * drawPageParameter.volumeRect.width - padding,
                drawPageParameter.volumeRect.yMin +
                (float)y / virtualTexture.pageSize * drawPageParameter.volumeRect.height - padding,
                drawPageParameter.volumeRect.width / virtualTexture.pageSize * perSize + 2f * padding,
                drawPageParameter.volumeRect.width / virtualTexture.pageSize * perSize + 2f * padding);
            ...
            foreach (var terrain in drawPageParameter.terrainList)
            {
                ...
                if (!volumeRect.Overlaps(terrainRect))
                {
                    continue;
                }
                ...
                FRect offsetRect = new FRect(pageRect.x + (maxRect.xMin - volumeRect.xMin) * scaleFactor,
                    pageRect.y + (maxRect.yMin - volumeRect.yMin) * scaleFactor, maxRect.width * scaleFactor,
                    maxRect.height * scaleFactor);
                ...
                float4 scaleOffset = new float4(maxRect.width / terrainRect.width, maxRect.height / terrainRect.height,
                    (maxRect.xMin - terrainRect.xMin) / terrainRect.width,
                    (maxRect.yMin - terrainRect.yMin) / terrainRect.height);
                m_Property.Clear();
                ...
                for (int i = 0; i < terrain.terrainData.alphamapTextureCount; ++i)
                {
                    ...
                    for (; layerIndex < terrain.terrainData.alphamapLayers && index <= 4; ++layerIndex)
                    {
                        ...
                        float2 tileScale = new float2(terrain.terrainData.size.x / terrainLayer.tileSize.x,
                            terrain.terrainData.size.z / terrainLayer.tileSize.y);
                        float4 tileOffset = new float4(tileScale.x * scaleOffset.x, tileScale.y * scaleOffset.y,
                            scaleOffset.z * tileScale.x, scaleOffset.w * tileScale.y);
                    ...
                    cmdBuffer.DrawMesh(m_DrawPageMesh, Matrix4x4.identity, m_DrawColorMaterial, 0,
                        layerIndex <= 4 ? 0 : 1, m_Property);
                }
            }
        }
