...
namespace RVT
{
    ...
    public struct FRect : IEquatable<FRect>, IFormattable
    {
        ...
        public static FRect MinMaxRect(float xmin, float ymin, float xmax, float ymax) =>
            new FRect(xmin, ymin, xmax - xmin, ymax - ymin);
        ...
        public bool Contains(float2 point) => (double)point.x >= (double)this.xMin &&
                                              (double)point.x < (double)this.xMax &&
                                              (double)point.y >= (double)this.yMin &&
                                              (double)point.y < (double)this.yMax;

        public bool Contains(float3 point) => (double)point.x >= (double)this.xMin &&
                                              (double)point.x < (double)this.xMax &&
                                              (double)point.y >= (double)this.yMin &&
                                              (double)point.y < (double)this.yMax;

        public bool Contains(float3 point, bool allowInverse) => !allowInverse
            ? this.Contains(point)
            : ((double)this.width < 0.0 && (double)point.x <= (double)this.xMin &&
               (double)point.x > (double)this.xMax ||
               (double)this.width >= 0.0 && (double)point.x >= (double)this.xMin &&
               (double)point.x < (double)this.xMax) &
              ((double)this.height < 0.0 && (double)point.y <= (double)this.yMin &&
                  (double)point.y > (double)this.yMax || (double)this.height >= 0.0 &&
                  (double)point.y >= (double)this.yMin && (double)point.y < (double)this.yMax);
        ...
        public bool Overlaps(FRect other) => (double)other.xMax > (double)this.xMin &&
                                             (double)other.xMin < (double)this.xMax &&
                                             (double)other.yMax > (double)this.yMin &&
                                             (double)other.yMin < (double)this.yMax;
        ...
        public static float2 PointToNormalized(FRect rectangle, float2 point) => new float2(
        ...
        public static bool operator ==(FRect lhs, FRect rhs) => (double)lhs.x == (double)rhs.x &&
                                                                (double)lhs.y == (double)rhs.y &&
                                                                (double)lhs.width == (double)rhs.width &&
                                                                (double)lhs.height == (double)rhs.height;
        ...
        public override string ToString() =>
            this.ToString((string)null, (IFormatProvider)CultureInfo.InvariantCulture.NumberFormat);

        public string ToString(string format) =>
            this.ToString(format, (IFormatProvider)CultureInfo.InvariantCulture.NumberFormat);
    ...
    public struct FRectInt : IEquatable<FRectInt>, IFormattable
    {
        ...
        public float2 center => new float2((float)this.x + (float)this.m_Width / 2f,
            (float)this.y + (float)this.m_Height / 2f);
        ...
        {
            this.position = new int2(Math.Max(Math.Min(bounds.xMax, this.position.x), bounds.xMin),
                Math.Max(Math.Min(bounds.yMax, this.position.y), bounds.yMin));
            this.size = new int2(Math.Min(bounds.xMax - this.position.x, this.size.x),
                Math.Min(bounds.yMax - this.position.y, this.size.y));
        }

        public bool Contains(int2 position) => position.x >= this.xMin && position.y >= this.yMin &&
                                               position.x < this.xMax && position.y < this.yMax;

        public bool Overlaps(FRectInt other) => other.xMin < this.xMax && other.xMax > this.xMin &&
        ...
        public string ToString(string format, IFormatProvider formatProvider) => string.Format(
            "(x:{0}, y:{1}, width:{2}, height:{3})", (object)this.x.ToString(format, formatProvider),
            (object)this.y.ToString(format, formatProvider), (object)this.width.ToString(format, formatProvider),
            (object)this.height.ToString(format, formatProvider));

        public bool Equals(FRectInt other) => this.m_XMin == other.m_XMin && this.m_YMin == other.m_YMin &&
