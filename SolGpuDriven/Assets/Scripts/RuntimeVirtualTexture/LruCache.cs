...
namespace RVT
{
    ...
    public unsafe struct FLruCache : IDisposable
    {
        internal int length;
        ...
        internal int First
        {
            get { return headNodeInfo.id; }
        }

        public FLruCache(in int length)
        {
            this.length = length;
            this.nodeInfoList = (FNodeInfo*)UnsafeUtility.Malloc(Marshal.SizeOf(typeof(FNodeInfo)) * length, 64,
