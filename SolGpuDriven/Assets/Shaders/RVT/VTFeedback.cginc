...
{
    //_VTFeedbackParam.x : PageTable's TableSize
    float2 page = floor(i.uv * _VTFeedbackParam.x);
    //_VTFeedbackParam.y virtual texture size
    float2 uv = i.uv * _VTFeedbackParam.y;
    float2 dx = ddx(uv);
    float2 dy = ddy(uv);
    int mip = clamp(int(0.5*log2(max(dot(dx,dy),dot(dy,dy))) + 0.5 + _VTFeedbackParam.w),0,_VTFeedbackParam.z);

    //return r : pageIdx, g : mipLevel,
    return float4(page / 255.0f,mip / 255.0f,1.0f);
    //return float4(0.0f,0.0f,mip / _VTFeedbackParam.z,1.0f);
    //return float4(page / 255.0f,0.0f,1.0f);
}
